...
function EditBusinessHour(props) {
  ...
  useEffect(
    function () {
      ...
      const isValidInteger = /^\d+$/.test(business_hour_id);
      if (!isValidInteger) {
        navigate("/home");
        return;
      }
      get_data(
        ...
        if (data) {
          setBh(data.data.data);
        }
      });
    },
    [appContext.reload, business_hour_id, navigate]
  );
  function handleSubmit() {
    console.log(bh);
    let payload = { data: bh, business_hour_id: business_hour_id };
